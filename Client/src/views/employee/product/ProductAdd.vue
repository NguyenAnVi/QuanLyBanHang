  ...
  methods: {
    async onEditorChangeHandler($event){
      ...
      setTimeout(() => {
        ...
        document.getElementById('description').dispatchEvent(new Event('change'));
...
<template>
  <main>
    <div class="wrapper">
      ...
      <UploadImage @submitCroppedImage="addNewImage" :aspectRatio="1" @close="closeModal" id="uploadImageModal"/>
    </div>
  </main>
...
.section-wrapper {
  ...
  & > .group-wrapper {
    ...
    & .cell-wrapper {
      ...
      & input, & :autofill, & :-internal-autofill-selected {
        width: 100%;
        padding: 10px 0px;
        border: none;
